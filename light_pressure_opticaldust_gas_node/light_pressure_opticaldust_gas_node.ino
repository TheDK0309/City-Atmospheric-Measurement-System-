...
void do_send(osjob_t* j){
  ...
  if (LMIC.opmode & OP_TXRXPEND) {
    ...
    String temporary = "";
    temporary = String(pressure) + String('|') + String(on_fire) + String('|') + String(lat_val) + String('|') + String(lng_val) + String('|') + String(dustDensity) + String('|') + String(light)+ String(' ');
...
{
  smoke_value = analogRead(smoke);
  smoke_value = constrain(smoke_value, 0, 1023);
  Serial.println(smoke_value); //print our value
  if(smoke_value>sensorThres){
    on_fire=true;
  }
  else{
    on_fire=false;
